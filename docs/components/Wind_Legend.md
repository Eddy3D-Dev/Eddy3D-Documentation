...
| ---- | -------- | ----------- | ---- |
| Range | R | Speed range the legend spans (m/s). Wire the Vector Field Viewer's Range output — that is the resolved range its colors actually map — or construct a domain to set the scale manually. | `Domain` |
