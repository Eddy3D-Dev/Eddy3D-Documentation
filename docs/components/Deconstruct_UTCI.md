...
| ---- | -------- | ----------- | ---- |
| Result | R | Result output of the UTCI (Simulation) component. | `Generic Data` |
...
| Min |  | Lowest UTCI (°C) per probe over the selected hours. | `Number` |
| Max |  | Highest UTCI (°C) per probe over the selected hours. | `Number` |
...
| UTCI | U | UTCI (°C) at each selected hour — one branch per HOY, each holding every probe's value {hoy}(probe count). Populated ONLY when HOY is connected; with HOY unconnected this would be the entire year, which is the tree this component exists to avoid. | `Number` |
