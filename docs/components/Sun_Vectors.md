...
![](/images/components/Sun_Vectors-crop.png)
...
| ---- | -------- | ----------- | ---- |
| Weather | W | Optional weather object; Latitude, Longitude and Time Zone are read from it when connected, and override the numeric inputs below. | `Generic Data` |
...
| From Month | FM | Start month [1-12]. | `Integer` |
| From Day | FD | Start day [1-31]. | `Integer` |
| To Month | TM | End month [1-12], inclusive. | `Integer` |
| To Day | TD | End day [1-31], inclusive. | `Integer` |
| Start Hour | SH | First hour of day [0-23]. | `Integer` |
| End Hour | EH | Last hour of day, exclusive [1-24]. | `Integer` |
...
| Name | Nickname | Description | Type |
| ---- | -------- | ----------- | ---- |
| Sun Vectors | SV | Sun samples — direction, the hours each represents, and the instant sampled. Feeds Sun Path. The analysis components read a weather record directly instead. | `Generic Data` |
