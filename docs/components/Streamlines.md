...
| Color Map | CM | Color ramp for the speed coloring — pick the same map as the Wind Field Viewer / Wind Legend. | `Text` |
| Range | R | Optional speed range [min, max] (m/s) to lock the color scale. Empty = the tracks' own min/max. Wire the Wind Field Viewer's Range output (or another Streamlines component's) to color both on one comparable scale. | `Domain` |
...
| Speeds | Spd | Velocity magnitude along each track; branch = {sub-result, track}, items = one per vertex (feed with Streamlines into Wind Field Viewer or a gradient for coloring). | `Number` |
