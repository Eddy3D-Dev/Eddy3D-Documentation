...
| Color Map | CM | Color ramp for the speed coloring — pick the same map as the Vector Field Viewer / Flex Legend. | `Text` |
...
| Range | R | The speed range the colors span (m/s): the Range input where wired, else the tracks' min/max. Feed it to the Flex Legend component for a matching legend. | `Domain` |