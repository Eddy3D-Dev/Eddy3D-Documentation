...
| Color Map | CM | Color ramp — pick the same map as the Wind Legend so the legend matches what is drawn. | `Text` |
...
| Range | R | The value range the colors span: Min/Max Value where supplied, otherwise the data minimum/maximum. Feed it to the Wind Legend component for a matching legend, or into a second viewer's Min/Max to put two fields on one comparable scale. | `Domain` |