...
| Range | R | The speed range the colors span (m/s): Min/Max Speed where supplied, otherwise the data minimum/maximum. Feed it to the Wind Legend component for a matching legend, or into a second Wind Field Viewer's Min/Max Speed to color two fields on one scale. | `Domain` |